        ...
        if let Ok(relative_path) = entry.path().strip_prefix(orig_path) {
            if let Some(first_component) = relative_path.components().next() {
                if first_component.as_os_str() == "organized" {
                    ...
                    continue;
                } else if first_component.as_os_str() == ".DS_Store" {
                    println!("Entry {:?} has '.DS_Store' as the first subfolder, skipping", entry.path());
                    continue;
                }  else if first_component.as_os_str() == "potential_live_photo_videos" {
                    println!("Entry {:?} has 'potential_live_photo_videos' as the first subfolder, skipping", entry.path());
                    continue;
                } 
            }
